...
class TestUserModelViewSet(TestCase):
    ...
    def test_get_detail_guest(self):
        client = APIClient()
        response = client.get(f'/api/users/{self.user.uid}/')
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_edit_guest(self):
        client = APIClient()
        response = client.put(f'/api/users/{self.user.uid}/', self.user_data_upd)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_edit_admin(self):
        ...
        response = client.put(f'/api/users/{user.uid}/', self.user_data_upd)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
